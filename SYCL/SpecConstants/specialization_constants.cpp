...
int main(int argc, char **argv) {
  std::cout << "check specialization constants API. (seed ="
            << seed << "\n";
  ...
  try {
...

    {
      ...
      q.submit([&](handler &cgh) {
        ...
        cgh.single_task<SpecializedKernel>(
            prog.get_kernel<SpecializedKernel>(),
            [=]() {
              bool_acc  [0] = i1.get();
              int8_acc  [0] = i8.get();
              uint8_acc [0] = ui8.get();
    ...
    if (!check(bool_test,   bool_ref,   "bool")) return 1;
    if (!check(int8_test,   int8_ref,   "int8")) return 1;
    if (!check(uint8_test,  uint8_ref,  "uint8")) return 1;
    if (!check(int16_test,  int16_ref,  "int16")) return 1;
    if (!check(uint16_test, uint16_ref, "uint16")) return 1;
    if (!check(int32_test,  int32_ref,  "int32")) return 1;
    if (!check(uint32_test, uint32_ref, "uint32")) return 1;
    if (!check(int64_test,  int64_ref,  "int64")) return 1;
    if (!check(uint64_test, uint64_ref, "uint64")) return 1;
#if HALF
    if (!check(half_test,   half_ref,   "half")) return 1;
#endif
    if (!check(float_test,  float_ref,  "float")) return 1;
    if (!check(double_test, double_ref, "double")) return 1;
