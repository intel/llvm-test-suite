...
void test_unnormalized_none_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 4; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                test_acc[i++] = image_acc.read(0, UnNorm_None_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(1, UnNorm_None_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(2, UnNorm_None_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(3, UnNorm_None_Nearest_sampler); // {11,12,13,14}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=0; std::cout << "read four pixels,  sample:   NonNormalized +  None  + Nearest" << std::endl; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_unnormalized_none_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
