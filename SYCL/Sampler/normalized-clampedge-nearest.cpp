...
void test_normalized_clampedge_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 7; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        //sampler
        auto Norm_ClampEdge_Nearest_sampler      = sampler(normalized, clamp_edge, nearest);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_norm_nearest>([=] () {
                ...
                //0-3 read four pixels at low-boundary locations,  sample:   Normalized +  Clamp To Edge  + Nearest
                test_acc[i++] = image_acc.read(0.00f, Norm_ClampEdge_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(0.25f, Norm_ClampEdge_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(0.50f, Norm_ClampEdge_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(0.75f, Norm_ClampEdge_Nearest_sampler); // {11,12,13,14}

                //4-6 read three pixels out of buonds,   sample:   Normalized + Clamp To Edge + Nearest
                test_acc[i++] = image_acc.read(-0.125f, Norm_ClampEdge_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(1.0f, Norm_ClampEdge_Nearest_sampler);    // {11,12,13,14}
                test_acc[i++] = image_acc.read(1.125f, Norm_ClampEdge_Nearest_sampler);  // {11,12,13,14}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=0; std::cout << "read four pixels at low-boundary locations,  sample:   Normalized +  Clamp To Edge  + Nearest" << std::endl; }
            if(i==4){ idx=-1; std::cout << "read three pixels out of buonds,   sample:   Normalized + Clamp To Edge + Nearest" << std::endl; }
            if(i==5){ idx=4; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_normalized_clampedge_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
...
//CHECK-NEXT: read three pixels out of buonds,   sample:   Normalized + Clamp To Edge + Nearest
//CHECK-NEXT: 4 -- -1: {1,2,3,4} 
//CHECK-NEXT: 5 -- 4: {11,12,13,14} 
//CHECK-NEXT: 6 -- 5: {11,12,13,14} 