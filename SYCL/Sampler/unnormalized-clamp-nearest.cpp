...
void test_unnormalized_clamp_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    ...
    { // closure
        ...
        //sampler
        auto UnNorm_Clamp_Nearest_sampler      = sampler(unnormalized, addressing_mode::clamp, nearest);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                test_acc[i++] = image_acc.read(-1, UnNorm_Clamp_Nearest_sampler); // {0,0,0,0}
                test_acc[i++] = image_acc.read(0, UnNorm_Clamp_Nearest_sampler);  // {1,2,3,4}
                test_acc[i++] = image_acc.read(1, UnNorm_Clamp_Nearest_sampler);  // {5,6,7,8}
                test_acc[i++] = image_acc.read(2, UnNorm_Clamp_Nearest_sampler);  // {5,6,7,8}
                test_acc[i++] = image_acc.read(3, UnNorm_Clamp_Nearest_sampler);  // {9,10,11,12}
                test_acc[i++] = image_acc.read(4, UnNorm_Clamp_Nearest_sampler);  // {0,0,0,0}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=-1; std::cout << "read six pixels,   sampler:   UnNormalized + Clamp + Nearest" << std::endl; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_unnormalized_clamp_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
