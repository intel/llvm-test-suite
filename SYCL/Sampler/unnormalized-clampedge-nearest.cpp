...
void test_unnormalized_clampedge_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 6; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        //sampler
        auto UnNorm_ClampEdge_Nearest_sampler      = sampler(unnormalized, clamp_edge, nearest);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                test_acc[i++] = image_acc.read(-1, UnNorm_ClampEdge_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(0, UnNorm_ClampEdge_Nearest_sampler);  // {1,2,3,4}
                test_acc[i++] = image_acc.read(1, UnNorm_ClampEdge_Nearest_sampler);  // {5,6,7,8}
                test_acc[i++] = image_acc.read(2, UnNorm_ClampEdge_Nearest_sampler);  // {5,6,7,8}
                test_acc[i++] = image_acc.read(3, UnNorm_ClampEdge_Nearest_sampler);  // {9,10,11,12}
                test_acc[i++] = image_acc.read(4, UnNorm_ClampEdge_Nearest_sampler);  // {9,10,11,12}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=-1; std::cout << "read six pixels,   sampler:   UnNormalized + ClampEdge + Nearest" << std::endl; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_unnormalized_clampedge_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
