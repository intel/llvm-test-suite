...
void test_unnormalized_clampedge_linear_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    ...
    { // closure
        ...
        //sampler
        auto UnNorm_ClampEdge_Linear_sampler      = sampler(unnormalized, clamp_edge, linear);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                //0-5 read six pixels, float coordinates,   sample:   NonNormalized + ClampEDGE + Linear
                test_acc[i++] = image_acc.read(-1.0f, UnNorm_ClampEdge_Linear_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(0.0f, UnNorm_ClampEdge_Linear_sampler);  // {1,2,3,4}
                test_acc[i++] = image_acc.read(1.0f, UnNorm_ClampEdge_Linear_sampler);  // {25,25,25,25} //interpolated
                test_acc[i++] = image_acc.read(2.0f, UnNorm_ClampEdge_Linear_sampler);  // {54,53,52,51}
                test_acc[i++] = image_acc.read(3.0f, UnNorm_ClampEdge_Linear_sampler);  // {35,35,35,35} 
                test_acc[i++] = image_acc.read(4.0f, UnNorm_ClampEdge_Linear_sampler);  // {11,12,13,14} 

                //6-7 read two pixels on either side of 1. float coordinates. ClampEDGE
                //  ClampEDGE correctly interpolates 
                test_acc[i++] = image_acc.read(0.9999f, UnNorm_ClampEdge_Linear_sampler); // {25,25,25,25}
                test_acc[i++] = image_acc.read(1.0001f, UnNorm_ClampEdge_Linear_sampler); // {25,25,25,25}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=-1; std::cout << "read six pixels, float coordinates,   sample:   NonNormalized + ClampEDGE + Linear" << std::endl; }
            if(i==6){ idx=1; std::cout << "read two pixels on either side of 1. float coordinates. ClampEDGE" << std::endl; }
            if(i==7){ idx=1;  }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_unnormalized_clampedge_linear_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
