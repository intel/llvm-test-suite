...
void test_normalized_mirrored_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 12; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_norm_nearest>([=] () {
                ...
                //0-3 read four pixels at low-boundary locations,  sample:   Normalized + Mirrored Repeat  + Nearest
                test_acc[i++] = image_acc.read(0.00f, Norm_Mirror_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(0.25f, Norm_Mirror_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(0.50f, Norm_Mirror_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(0.75f, Norm_Mirror_Nearest_sampler); // {11,12,13,14}

                //4-7 read four pixels outside rightmost boundary,   sample: Normalized + Mirrored Repeat + Nearest
                test_acc[i++] = image_acc.read(1.875f, Norm_Mirror_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(1.625f, Norm_Mirror_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(1.375f, Norm_Mirror_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(1.125f, Norm_Mirror_Nearest_sampler); // {11,12,13,14}
                //8-11 read four pixels outside leftmost boundary,   sample: Normalized + Mirrored Repeat + Nearest
                test_acc[i++] = image_acc.read(-0.125f, Norm_Mirror_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(-0.375f, Norm_Mirror_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(-0.625f, Norm_Mirror_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(-0.875f, Norm_Mirror_Nearest_sampler); // {11,12,13,14}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=0; std::cout << "read four pixels at low-boundary locations,  sample:   Normalized + Mirrored Repeat  + Nearest" << std::endl; }
            if(i==4){ idx=0; std::cout << "read four pixels outside rightmost boundary,   sample: Normalized + Mirrored Repeat + Nearest" << std::endl; }
            if(i==8){ idx=0; std::cout << "read four pixels outside leftmost boundary,   sample: Normalized + Mirrored Repeat + Nearest" << std::endl; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_normalized_mirrored_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
...
//CHECK-NEXT: read four pixels outside rightmost boundary,   sample: Normalized + Mirrored Repeat + Nearest
//CHECK-NEXT: 4 -- 0: {1,2,3,4} 
//CHECK-NEXT: 5 -- 1: {49,48,47,46} 
//CHECK-NEXT: 6 -- 2: {59,58,57,56} 
//CHECK-NEXT: 7 -- 3: {11,12,13,14} 
//CHECK-NEXT: read four pixels outside leftmost boundary,   sample: Normalized + Mirrored Repeat + Nearest
