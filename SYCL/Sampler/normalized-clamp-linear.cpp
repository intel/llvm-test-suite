...
void test_normalized_clamp_linear_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 9; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        //sampler
        auto Norm_Clamp_Linear_sampler      = sampler(normalized, addressing_mode::clamp, linear);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_norm_linear>([=] () {
                ...
                //0-6 read seven pixels at 'boundary' locations, starting out of bounds,  sample:   Normalized +  Clamp  + Linear
                test_acc[i++] = image_acc.read(-0.25f, Norm_Clamp_Linear_sampler); // {0,0,0,0}
                test_acc[i++] = image_acc.read(0.00f, Norm_Clamp_Linear_sampler);  // {0,1,2,2} // interpolating with bg color. consistent with unnormalized.  Doesn't seem 100% correct to me, but don't ahve anything to compare against presnetly
                test_acc[i++] = image_acc.read(0.25f, Norm_Clamp_Linear_sampler); // {25,25,25,25}
                test_acc[i++] = image_acc.read(0.50f, Norm_Clamp_Linear_sampler); // {54,53,52,51}
                test_acc[i++] = image_acc.read(0.75f, Norm_Clamp_Linear_sampler); // {35,35,35,35}
                test_acc[i++] = image_acc.read(1.00f, Norm_Clamp_Linear_sampler); // {6,6,6,7}  // interpolating with bg
                test_acc[i++] = image_acc.read(1.25f, Norm_Clamp_Linear_sampler); // {0,0,0,0}

                //7-8 read two pixels on either side of first pixel. float coordinates. CLAMP
                //  on GPU CLAMP is apparently stopping the interpolation. ( values on right are expected value)
                test_acc[i++] = image_acc.read(0.2499f, Norm_Clamp_Linear_sampler); // {25,25,25,25}
                test_acc[i++] = image_acc.read(0.2501f, Norm_Clamp_Linear_sampler); // {25,25,25,25}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=-1; std::cout << "read six pixels at 'boundary' locations, starting out of bounds,  sample:   Normalized +  Clamp  + Linear" << std::endl; }
            if(i==7){ idx=1; std::cout << "read two pixels on either side of first pixel. float coordinates. Normalized +  Clamp  + Linear" << std::endl; }
            if(i==8){ idx=1; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_normalized_clamp_linear_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
