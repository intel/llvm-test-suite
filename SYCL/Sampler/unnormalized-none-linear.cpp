...
void test_unnormalized_none_linear_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    int numTests = 11; // drives the size of the testResults buffer, and the number of report iterations. Kludge.
    ...
    { // closure
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                //0-2 read three pixels at inner boundary locations, float coordinates,  sample:   UnNormalized +  None  + Linear 
                test_acc[i++] = image_acc.read(1.0f, UnNorm_None_Linear_sampler); // {25,25,25,25}
                test_acc[i++] = image_acc.read(2.0f, UnNorm_None_Linear_sampler); // {54,53,52,51}
                test_acc[i++] = image_acc.read(3.0f, UnNorm_None_Linear_sampler); // {35,35,35,35}

                //3-6 read four pixels at exact center locations, float,  sample:   Unnormalized +  None  + Linear
                test_acc[i++] = image_acc.read(0.5f, UnNorm_None_Linear_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(1.5f, UnNorm_None_Linear_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(2.5f, UnNorm_None_Linear_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(3.5f, UnNorm_None_Linear_sampler); // {11,12,13,14}

                //7-10 read four pixels at inexact upper boundary, float coord,  sample:   Unnormalized +  None  + Linear
                test_acc[i++] = image_acc.read(0.9999f, UnNorm_None_Linear_sampler); // {25,25,25,25}
                test_acc[i++] = image_acc.read(1.9999f, UnNorm_None_Linear_sampler); // {54,53,52,51}
                test_acc[i++] = image_acc.read(2.9999f, UnNorm_None_Linear_sampler); // {35,35,35,35}
                test_acc[i++] = image_acc.read(3.9999f, UnNorm_None_Linear_sampler); // {6,6,7,7}  // <<--- should it be interpolating with the bg color? That doesn't seem right. But it is what it is.
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=1; std::cout << "read three pixels at inner boundary locations, float coordinates,  sample:   UnNormalized +  None  + Linear" << std::endl; }
            if(i==3){ idx=0; std::cout << "read four pixels at exact center locations, float,  sample:   Unnormalized +  None  + Linear" << std::endl; }
            if(i==7){ idx=0; std::cout << "read four pixels at inexact upper boundary, float coord,  sample:   Unnormalized +  None  + Linear" << std::endl; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_unnormalized_none_linear_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
