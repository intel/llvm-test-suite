...
/*
    This file sets up an image, initializes it with data, and verifies that the data can be read directly.
    ...
    SYCL_DEVICE_FILTER=opecl:acc ../binx.bin    <--  does not support image operations at this time.
    ...
    { // closure
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_Unorm_Linear>([=] () {
                ...
                //Add more tests below. Just be sure to increase the numTests counter at the beginning of this function
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=0; std::cout << "read four pixels, no sampler" << std::endl; }
