...
void test_normalized_clamp_nearest_sampler(image_channel_order ChanOrder, image_channel_type ChanType){
    ...
    { // closure
        ...
        //sampler
        auto Norm_Clamp_Nearest_sampler      = sampler(normalized, addressing_mode::clamp, nearest);
        ...
        event E_Test = Q.submit([&](handler &cgh){
            ...
            cgh.single_task<class im1D_norm_nearest>([=] () {
                ...
                //0-3 read four pixels at low-boundary locations,  sample:   Normalized +  Clamp  + Nearest
                test_acc[i++] = image_acc.read(0.00f, Norm_Clamp_Nearest_sampler); // {1,2,3,4}
                test_acc[i++] = image_acc.read(0.25f, Norm_Clamp_Nearest_sampler); // {49,48,47,46}
                test_acc[i++] = image_acc.read(0.50f, Norm_Clamp_Nearest_sampler); // {59,58,57,56}
                test_acc[i++] = image_acc.read(0.75f, Norm_Clamp_Nearest_sampler); // {11,12,13,14}

                //4-6 read three pixels out of bounds,   sample:   Normalized + Clamp + Nearest
                test_acc[i++] = image_acc.read(-0.125f, Norm_Clamp_Nearest_sampler); // {0,0,0,0}
                test_acc[i++] = image_acc.read(1.0f, Norm_Clamp_Nearest_sampler);    // {0,0,0,0}
                test_acc[i++] = image_acc.read(1.125f, Norm_Clamp_Nearest_sampler);  // {0,0,0,0}
        ...
        for(int i = 0, idx=0; i < numTests; i++, idx++){
            if(i==0){ idx=0; std::cout << "read four pixels at low-boundary locations,  sample:   Normalized +  Clamp  + Nearest" << std::endl; }
            if(i==4){ idx=-1; std::cout << "read three pixels out of bounds,   sample:   Normalized + Clamp + Nearest" << std::endl; }
            if(i==5){ idx=4; }
...
int main(){
    ...
    if(D.has(aspect::image)){ 
        ...
        test_normalized_clamp_nearest_sampler(image_channel_order::rgba, image_channel_type::unsigned_int8);
    } else {
